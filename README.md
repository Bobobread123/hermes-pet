...
> 说明：原计划所有桌宠调用都带 `--source tool` 做会话隔离；2026-04-29 实测该开关与 `-Q` 组合会显著变慢，V1 暂时不带，优先保证交互延迟。
...
- ✅ 第一周 spike：Tauri 透明窗口骨架 + Hermes runner 端到端已跑通
- ⏳ 第一周 spike：鼠标穿透、多窗口、SVG 角色继续推进
...
- [`docs/progress.md`](docs/progress.md) —— 实现进度与排障记录（例如 Hermes Runner / Tauri event 踩坑）
