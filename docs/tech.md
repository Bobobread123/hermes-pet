...
| Hermes 接入方式 | `hermes chat -q "..." -Q --source tool` 子进程模型（详见 §3） |
...
| `--source tool` | 给会话打"非用户"标签 | **必须开**，让桌宠的会话不污染 `hermes sessions list` |
...
|---|---|---|---|
| **research** | settings.system_prompts.research | 单轮（每次新 session） | `hermes chat -Q --source tool --accept-hooks -q "<system_prompt>\n\n<user_input>"` |
| **对话** | settings.system_prompts.dialog（可被气泡内"临时改"覆盖） | 多轮（首次 spawn 拿 session_id，后续 `-r <id>`） | 首次同上；后续：`hermes chat -Q --source tool --accept-hooks -r <session_id> -q "<user_input>"` |
...
- ⚠️ 待 spike 验证：`-Q` 模式下 stdout 是**逐行流式**输出，还是**攒完一次性**输出？
  - 如果是流式 → 直接当 stream 喂前端
  - 如果是一次性 → 红脸期间 UI 不显示中间内容（research/cowork 本来就是结果型，能接受；对话气泡需要看流，可能要 fallback 到 `--verbose` 解析）

桌宠端的处理（无论哪种）：
- spawn → 立即广播 `task_started` 事件 → 红脸开
- stdout 每收到一段 → 广播 `task_stream { task_id, chunk }` → 前端按气泡 ID 累积
- 子进程退出 → 广播 `task_done { task_id, exit_code, full_output }` → 红脸消、挥手
- exit_code != 0 或 stderr 非空 → `task_error`，桌宠困惑表情
...
```bash
<hermes_bin> chat -Q --source tool --accept-hooks -q "ping, reply with 'pong' only"
```

- exit 0 + stdout 含 "pong" → ✅
- 失败 → 显示 stderr 全文 + 常见原因清单

### 4.4 默认 `--source tool` 隔离

所有桌宠发起的会话都打 `--source tool` 标签，这样：
...
- 对话气泡首次提交：spawn 不带 `-r`，从 stdout 末尾解析出 session_id（Hermes 退出时会打印），存到内存
...
> ⚠️ 待 spike 验证：`-Q` 模式下 session_id 是否依然在 stdout 输出？如果被静默吞了，需要走 `hermes sessions list -n 1 --source tool` 拿最近一个。
...
│  │       ├─ stdout reader → emit "stream"             │
│  │       ├─ stderr reader → emit "stderr"             │
...
1. 用户在 cowork 气泡按回车 → 前端 `invoke('submit_task', { bubble: 'cowork', input: '...' })`
2. Rust 端拼好命令，spawn，立即返回 `task_id`
3. 前端 taskMachine 进入 `running` → 通知 modeMachine `running_task_count++` → 桌宠红脸
4. Rust 端持续 `emit('hermes:stream', { task_id, chunk })` → 前端按 task_id 累积到对应气泡的 buffer
5. Rust 端 `wait` 到子进程退出 → `emit('hermes:done', { task_id, exit_code })`
6. 前端 taskMachine → `done`，modeMachine `running_task_count--`，归 0 → 红脸消 + 挥手 + 气泡红点
...
│       └── events.ts             # 监听 hermes:stream / done / error
├── assets/
│   ├── sounds/
│   │   ├── pat.wav
│   │   ├── snore.wav
...
2. Tauri command `submit_task` 跑通：前端 invoke → 后端 spawn `hermes chat -Q --source tool -q "hello"` → stream 事件回到前端 console.log
