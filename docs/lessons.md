...
| 像素角色用 `<img src>` 渲染时像素块之间出现缝隙 | 缩放时亚像素对齐偏差导致相邻 rect 之间漏出背景色，即使加了 `image-rendering: pixelated` 也无法完全消除 | SVG 是 1200×550 按 50px 格子绘制的像素画，缩放到 180px 时产生浮点误差 | 改为 inline SVG（直接在 JSX 中写 `<svg>` 标签），加 `shapeRendering="crispEdges"`，矢量直接渲染不经图片缩放，完全无缝 |
