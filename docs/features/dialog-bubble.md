...
- 输入提交 → 红脸（任务运行）
- 流式输出过来的内容：**实时累积显示**在气泡下方的"对话 history" 浮窗里（与 research 不同，对话需要看到流的过程）
- 流结束 → 红脸消退 + 挥手
...
- V1：当前会话的 `session_id` 存桌宠内存。关掉桌宠 / 切普通模式 / 点 × → 丢掉 id，下次是新 session。Hermes Agent 那边的 session 数据其实还在（`~/.hermes/`），只是桌宠不再主动续接
