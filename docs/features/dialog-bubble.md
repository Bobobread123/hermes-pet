...
- 激活后的布局：对话输入 pill 移到三条 hover 入口中最下方（cowork 原位置），history 浮窗显示在其上方；整体交互高度以底部输入 pill / 文件 chip 区为准
- 浮窗位置：贴在对话气泡上方，宽度与下方对话输入 pill 保持一致
- 浮窗内：
  - Tab 区：当前气泡的 session tabs，可新建 / 切换 / 删除本地 session
  - 当前 session 的完整对话流
  - 右侧：用户每一次提问
  - 左侧：Hermes 每一次回复，运行中时最新回复气泡实时增长
  - 底部：与对话气泡输入框联动的输入区（实际同一个输入）
  - 顶部右上：放大 / 还原按钮（只改变当前浮窗尺寸，不改变 session）
  - 顶部右上：×（只关闭浮窗，不删除 session）
- 放大态：
  - 点击放大按钮后，history 浮窗扩展为更大的对话记录显示框（V1 固定大尺寸，约 640px 宽 / 560px 高）
  - 放大按钮使用简洁矩形 icon：小浮窗为 `□`，放大态为 `▣`
  - 放大态保留当前 session tabs、滚动位置和流式输出；再次点击按钮恢复小浮窗
  - 浮窗展开和放大 / 还原使用与输入 pill 同等级的 360ms 动效
  - 放大浮窗以右下角为锚点，向左上角扩展；Hermes 桌宠本体不移动
  - 放大浮窗会跟随气泡位置移动；纵向不做“完整留在屏幕内”的强制 clamp，避免拖动时大框卡住不下移
  - 放大浮窗需要同步扩大 Tauri hit region，避免透明窗口穿透导致无法点击浮窗内容
- 点击浮窗外不收起（避免对话被打断）；点 × 只关闭浮窗，删除要点 tab 上的 ×

## 历史记录

- V1：每个 tab 的 `session_id` 和**用于界面展示的消息列表**存在桌宠内存；上下文判断仍完全交给 Hermes session，不靠桌宠重放历史
- 新建 tab = 新的桌宠本地 session；删除 tab = 删除桌宠本地展示记录，不删除 Hermes Agent 那边的 session 数据
- 关掉桌宠 / 切普通模式 → 丢掉内存里的 tab 列表。Hermes Agent 那边的 session 数据其实还在（`~/.hermes/`），只是桌宠不再主动续接
